...
import {
  ...
  formatUnitCount,
  canViewResourceForRole,
  getResourceDetailRows,
  getComparableCapacityValue,
  getCapacityDisplayValue,
  getStatusMeta,
} from "./components/facilities/facilityBrowseCore.js";
      ...
      try {
        ...
        const hint =
          err?.message === "Failed to fetch"
            ? " Is the Spring Boot app running on port 8081? In dev, use `npm run dev` so /api is proxied."
            : "";
        setResourceError(
          err?.message
            ? `Could not load resources: ${err.message}.${hint}`
            : "Could not load resources from backend",
        );
      } finally {
                              ...
                              >
                                ...
                                {!isReadOnlyRole ? (
                                  <button
                                    ...
                                    onClick={() => setBookingResource(item)}
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      if (getStatusMeta(room.resource.status).isBookable) {
                                        setBookingResource(room.resource);
                                      }
                                    }}
